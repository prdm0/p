...
Utilizando o conceito de laços/loops, crie um código C que calcula o fatorial de um número inteiro. Considere um inteiro assinado da forma `unsigned long int`. Lembre-se que o fatorial de $n$ é dado por:
...
Utilizando o conceito de laços, crie um programa em C que some apenas os números pares da sequência de números inteiros de 1 à 100.
...
Utilizando o conceito de laços, escreva um programa em C que aproxime o valor de $\mathrm{sin}(x)$, utilizando a série de potência baixo:

$$\mathrm{sin}(x)  \approx \sum_{n = 0}^{N} \frac{(-1)^nx^{2n + 1}}{(2n + 1)!}.$$
**Nota**: Você deverá escolher um valor de $N$, por exemplo, $N = 5$. Não escolha um valor grande para $N$, para não cair em erros numéricos! Aqui, suficiente uma aproximação com seis termos; $n = 0, 1, 2, 3, 4, 5$.

## Questão 5:

Escreva um programa em C, utilizando o conceito de laços, em que solicite ao usuário uma quantidade $n$ de números reais que serão digitados. Em cada iteração do laço, o programa deverá solicitar um número real ao usuário. Ao fim das $n$ iterações, o programa deverá informar a soma dos números digitados.
